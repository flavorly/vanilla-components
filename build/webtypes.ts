...
const distDir = path.resolve(baseDir, '../packages/vanilla-components/dist')
const docsDir = path.resolve(baseDir, '../documentation')
...
const computePropType = ({ type }) => {
  ...
  return ['Boolean', 'String', 'Number', 'Function', 'Object'].includes(type)
    ? type.toLowerCase()
    : type
}
...
const processComponentMeta = (meta, groupRef, docUrl) => {
  ...
  const tag = {
    ...
    'docs-url': docUrl,
    'description': `'${componentName}' - Vanilla Components - Component '${tagName}'`,
    'attributes': [],
  }
...
const processDirectiveMeta = (directiveMeta, docUrl) => {
  ...
  const attribute = {
    ...
    'required': false,
    'description': `${name} - Vanilla Components directive '${kebabCase(name)}'`,
    'doc-url': docUrl,
  }
